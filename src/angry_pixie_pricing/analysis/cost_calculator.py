...
from ..data.electricity_prices import get_electricity_prices
...
class CostCalculator:
    ...
    def __init__(self, 
        ...
        self._calculation_results: Optional[Dict[str, Any]] = None
    ...
    def _align_data(self, load_data: pd.DataFrame, price_data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Price' in price_data.columns:
            ...
            raise ValueError(f"Price column not found in data. Available columns: {price_data.columns.tolist()}")
    ...
    def _calculate_statistics(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        price_stats = {
            ...
            'weighted_avg_price_eur_kwh': (
                (data['price_eur_kwh'] * data['energy_kwh']).sum() / 
                data['energy_kwh'].sum()
            )
        }
        ...
        hourly_avg['avg_cost_per_kwh'] = (
            hourly_avg['energy_cost_eur'] / hourly_avg['energy_kwh']
        )
    ...
    def optimize_load_shifting(self, 
                              shiftable_percentage: float = 0.2,
                              max_shift_hours: int = 4) -> Dict[str, Any]:
        ...
        avg_original_price = (
            hourly_data['energy_cost_eur'].sum() / 
            hourly_data['energy_kwh'].sum()
        )
