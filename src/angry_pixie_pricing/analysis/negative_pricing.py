...
class NegativePricingAnalyzer:
    ...
    def analyze_seasonal_patterns(self, df: pd.DataFrame) -> dict[int | str, Any]:
        ...
        if seasonal_analysis:
            current_values = [
                data["progress_metrics"]["current_hours_per_day"]
                for data in seasonal_analysis.values()
                if "error" not in data["progress_metrics"]
            ]
            potential_values = [
                data["progress_metrics"]["theoretical_max_hours_per_day"]
                for data in seasonal_analysis.values()
                if "error" not in data["progress_metrics"]
            ]
            ...
            seasonal_analysis["yearly_summary"] = {
                ...
                "overall_progress_percentage": (yearly_avg_current / yearly_avg_potential) * 100
                if yearly_avg_potential > 0
                else 0,
            }
    ...
    def _find_max_consecutive_hours(self, boolean_series: pd.Series) -> int:
        ...
        values = boolean_series.to_numpy()
        max_consecutive = 0
...
def calculate_monthly_hours_timeseries(
    ...
    monthly_agg = (
        df_monthly.groupby("month_start")
        .agg(
            {
                "is_negative": "sum",
                "is_near_zero": "sum",
...
def calculate_solar_quarter_hours_timeseries(
    ...
    if df.empty:
        return pd.DataFrame(
            columns=["quarter_start", "quarter_name", "negative_hours", "near_zero_hours"],
        )
    ...
    quarter_agg = (
        df_solar.groupby(["quarter_start", "quarter_name"])
        .agg(
            {
                "is_negative": "sum",
                "is_near_zero": "sum",
...
def calculate_aggregated_hours_timeseries_with_severity(
    ...
    if aggregation_level == "daily":
        ...
        msg = f"Unknown aggregation level: {aggregation_level}"
