...
class RollingDuckAnalyzer:
    ...
    def multi_window_analysis(
        ...
        for window_days in windows:
            ...
            results[window_key] = self.calculate_rolling_duck_factor(
                df, window_days, step_days
            )
    ...
    def detect_seasonal_patterns(
        ...
        # Seasonal averages
        seasonal_avg = df.groupby(df['month'].map(self._get_season))['duck_factor'].agg(['mean', 'std', 'count']).reset_index()
        ...
        return {
            ...
            'peak_season': seasonal_avg.loc[seasonal_avg['mean'].idxmax(), 'season'] if not seasonal_avg.empty else None,
    ...
    def detect_trends(
        ...
        if len(duck_factors_df) < min_data_points:
            return {'error': f'Insufficient data points for trend analysis (need {min_data_points}, got {len(duck_factors_df)})'}
    ...
    def year_over_year_comparison(
        ...
        return {
            ...
            'peak_growth_year': annual_avg.loc[annual_avg['yoy_change'].idxmax(), 'year'] if not annual_avg.empty and 'yoy_change' in annual_avg.columns else None,
