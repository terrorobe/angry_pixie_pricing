...
class BillingReconstructedProfile(LoadProfile):
    ...
    def _apply_day_night_constraint(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        current_total = current_day_kwh + current_night_kwh
        ...
        if current_day_kwh > 0:
            day_adjustment = target_day_kwh / current_day_kwh
        else:
            day_adjustment = 1.0
            
        if current_night_kwh > 0:
            night_adjustment = target_night_kwh / current_night_kwh
        else:
            night_adjustment = 1.0
    ...
    def _apply_operating_hours(self, df: pd.DataFrame, operating_hours: Dict) -> pd.DataFrame:
        """Apply operating hours constraints."""
        for day, hours in operating_hours.items():
            if isinstance(day, int):  # Weekday number (0=Monday)
    ...
    def _apply_holiday_adjustments(self, df: pd.DataFrame, holiday_dates: List[datetime]) -> pd.DataFrame:
    ...
    @classmethod
    def from_billing_data(cls,
                         billing_dict: Dict[str, Any],
                         profile_type: Optional[ProfileType] = None) -> 'BillingReconstructedProfile':
